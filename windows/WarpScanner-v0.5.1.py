...
def fetch_ip(url):
    try:
        ...
        return "Unavailable"
def what_ip():
    ipv4, ipv6 = fetch_ip('http://v4.ipv6-test.com/api/myip.php'), fetch_ip('http://v6.ipv6-test.com/api/myip.php')
    c_ipv4, c_ipv6 = (get_ip_details(ip) if ip != "Unavailable" else "Unavailable" for ip in (ipv4, ipv6))
...
def Check_for_update():
    ...
    def fetch_version():
        ...
        for url in urls:
            try:
                return urllib.request.urlopen(url,timeout=1).read().decode('utf-8').strip()
            except Exception:
                return requests.get(url, timeout=30).text.strip()
        return ""
    version = fetch_version()
    print(version)
    return [version != "v0.5.1", version]
def check_up():
    def go():
        ...
        ctk.CTkLabel(root, font=("Helvetica", 12, "bold"), text=f"New version Available {ret[1]}").pack(fill=tk.X)
        ctk.CTkButton(root, font=("Helvetica", 12, "bold"), text="Click to update", command=Github_link).pack(fill=tk.X, side=tk.BOTTOM)
        root.mainloop()
    def go_thread():
...
def start():
    def check_settings():
        if not os.path.exists("warp_setting") or len(open("warp_setting").readlines()) != 16:
...
try:
    ...
    with open("warp_setting", "r") as f:
        num33 = f.readlines()
    interval_see, count_see, timeout_see, ping_range_see = map(lambda x: int(x.strip()), num33[4:8])
except Exception as E:
    messagebox.showerror("Error", f"An error has occurred!: \n {E}")
...
def mainactivity():
    ...
    def forget_widget_list(list:list):
        for i in list:
            i.pack_forget()
    def pack_widgets(widgets,padx=None, pady=None ,ipady=None,ipadx=None,anchor="center", side=None, fill=None):
        for widget in widgets:
            try:
                widget.pack(padx=padx,pady=pady,anchor=anchor,side=side,ipady=ipady,ipadx=ipadx, fill=fill)
            except Exception:
                pass
    ...
    def parse_configs(conifg,num=0,cv=1,hy2_path="hy2/config.yaml"): # nuitka: pragma: no cover
        ...
        if is_editing:
            ...
            if conifg.startswith("vless://") or conifg.startswith("vmess://") or conifg.startswith("trojan://"):
                ...
                TYPE_w=change_list_f(["tcp","kcp","ws","httpupgrade","splithttp","xhttp","h2","quic","grpc"],TYPE)
                if TYPE=="tcp" or TYPE=="kcp" or TYPE=="quic":
                    ...
                    MODE_w=change_list_f(["auto","packet-up", "stream-up", "stream-one"],MODE)
                if TYPE!="tcp":
                    if TYPE!="kcp":
            ...
            def save_normal_conf_form(conifg:str,num:int,remarks:ctk.CTkEntry):
                ...
                data = {
                    ...
                    "hy2_obfs_password": obfs_password.get().strip() if obfs_password else "",
                    "hy2_hop_interval": porthopinginterval.get() if porthopinginterval else "",
            ...
            def build_vless_config(data: dict) -> str:
                ...
                if data["network"] in ["tcp","ws","h2","httpupgrade","xhttp","splithttp"]:
                ...
                query = urllib.parse.urlencode({k: v for k, v in query_params.items() if v})
                return f"{base}?{query}#{data['tag']}" if query else f"{base}#{data['tag']}"
            def build_vmess_config(data: dict) -> str:
                ...
                encoded = base64.b64encode(json.dumps(vmess_data).encode("utf-8")).decode("utf-8")
            ...
            def check_type(typ: str):
                type_widgets = {
                    ...
                    "kcp": {
                        ...
                        "header_values": ["none", "srtp", "utp", "wechat-video", "dtls", "wireguard"]
                    },
                    "quic": {
                        "show": {
                ...
                for label_widget,entry in zip(config["show"]["labels"],config["show"]["entries"]):
            ...
            def check_security(sec :str):
                sec_widget= {
                    ...
                    "":{
                        ...
                        "hide":[sni_la,sni,fram_in_in_7,fp_la,fp,alpn_la,alpn,pbk_la,pbk,fram_in_in_8,sid_la,sid,spx_la,spx]
                    }
                }
            ...
            if conifg.startswith("vless://") or conifg.startswith("vmess://"):
                ...
                pack_widgets([porthopinginterval_la,porthopinginterval],fill=tk.X,side=tk.LEFT,padx=4)
                tls_la=ctk.CTkLabel(fram_in_in_6  ,  text="tls: ")
                tls_la.pack( padx=(20,0),side=tk.LEFT)
                tls.pack(fill=tk.X,side=tk.LEFT,padx=4)
                fram_in_in_6.pack(fill=tk.X,pady=5,ipady=5,padx=5)
    ...
    def conc():
        ...
        if concted:
            ...
            if is_hy2:
                print("kill hy2")
                subprocess.Popen(["taskkill" , "/F", "/IM" ,"hysteria.exe"],  stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL,startupinfo=startupinfo)
                is_hy2=False
            subprocess.Popen(["taskkill" , "/F", "/IM" ,"xray.exe"],  stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL,startupinfo=startupinfo)
        ...
        def s_xray():
            ...
            with open("xray/"+selec_sub.get(), "r",encoding="utf-8") as f:
                ...
                conf_simple= json.load(f) if slc_sub=="user_custom_confs" else f.readlines()
            ...
            with open("xray/config.json", "w") as f:
                f.write(parse_configs(conf_simple[wch_sel])) if not isinstance( conf_simple[wch_sel],dict) else json.dump(conf_simple[wch_sel],f)
                ...
                for ip, port, ping, loss_rate,jitter, combined_score in whichTuple:
                    ...
                    if loss_rate == 0.00 and ping != 0.0 and ping < 500.00:
                        ...
                        if ipchanged.startswith("hy2://") or ipchanged.startswith("hysteria2://"):
                            hy=subprocess.Popen (['hy2/hysteria.exe', 'client' ,'-c' , 'hy2/config.yaml'], stdin=subprocess.DEVNULL, stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL,startupinfo=startupinfo)
                        with open("xray/xrayErr.log", "w") as errFile:
                            xa=subprocess.Popen (['xray/xray.exe', 'run', '-c' ,f'xray/config{i}.json'], stdin=subprocess.DEVNULL, stdout=errFile,stderr=errFile,startupinfo=startupinfo )
                        if test_th(http5,i):
                            print("is ok")
                            oklist.append((ip, port, ping, loss_rate,jitter, combined_score))
                        subprocess.call(["taskkill", "/F", "/PID", str(xa.pid)],creationflags=subprocess.CREATE_NO_WINDOW)

                        if ipchanged.startswith("hy2://") or ipchanged.startswith("hysteria2://"):
                            subprocess.call(["taskkill", "/F", "/PID", str(hy.pid)],creationflags=subprocess.CREATE_NO_WINDOW)
    ...
    def wireguard_config_main(which_ip):
            ...
            def parse_ip_ranges(filename):
                ...
                if start_ips==[] and end_ips==[]:
                    return  ['188.114.96.0', '162.159.192.0', '162.159.195.0'] , ['188.114.99.224', '162.159.193.224', '162.159.195.224']
    ...
    def clean():
        try:
            ...
            with open("warp_setting", "r") as f2:
                ...
                separator = "," if len(warp_settings) > 2 and warp_settings[2] == "2" else "\n"
            clean_result = []
            ip_count = 0
            for line in lines:
                ...
                if ip_part:
                    ...
                    ip_count += 1
            with open("clean_result.txt", "w") as txt_file, open("clean_result.csv", "w") as csv_file:
    ...
    def check_up():
            ...
            def on_go():
                ...
                if ret[0]  == True:
                    ...
                    label_up=ctk.CTkLabel(rootupdate ,font=("Helvetica", 12, "bold"), text="New version v0.5.1:\nGood news:\n\nbetter Graphic\n\nfix Bugs\n\nBad news:\n\nNotihng:))")
                    label_up.pack(fill=tk.X)
...
#     labelmain.pack()
    ...
    pack_widgets([Setting_button,up_button,About_button,exit_button,label_arshia],pady=5,ipady=5,fill=tk.BOTH)
    ...
    def ratio_event():
        ...
        if radio_var.get()==1:
            ...
            pack_widgets([button,button1,button3],fill=tk.X,anchor="center" , pady=4,)
        else:
    ...
    button1.bind("<Enter>", hand2)
    button3.bind("<Enter>", hand2)
    pack_widgets([button,button1,button3],fill=tk.X,anchor="center" , pady=4,)    #################### vpn view
    def is_frag_or_wire(nn):
        ...
        if op=="wiregaurd config setting":
            ...
            forget_widgets(button_frame),forget_widgets(button_frame2),forget_widgets(button_frame3)
            pack_widgets([packets_la,packets,length_la,length,interval_la,interval,fakehost_la,fakehost,mux_la,mux,frag_la],padx=(20,0),fill=tk.X,side=tk.LEFT)
        elif op=="vpn setting":
            forget_widgets(button_frame),forget_widgets(button_frame2),forget_widgets(button_frame3)
            pack_widgets([perapp_la,enablelocaldns_la,enablefakedns_la,local_la,local,allowincrease,sniffing],fill=tk.X,pady=5,padx=5,side=tk.LEFT)
        elif op=="routing setting":
            forget_widgets(button_frame),forget_widgets(button_frame2),forget_widgets(button_frame3)
            pack_widgets([domainStrategy_la,domainStrategy,customRules_proxy_la,customRules_proxy,customRules_direct_la,customRules_direct,customRules_blocked_la,customRules_blocked],fill=tk.X,pady=5,padx=5,side=tk.LEFT)
        elif op=="advanced setting":
            forget_widgets(button_frame),forget_widgets(button_frame2),forget_widgets(button_frame3)
            pack_widgets([socks5_la,socks5,http_la,http,remotedns_la,remotedns,domesticdns_la,domesticdns,loglevel_la,loglevel,test_link_la,test_link],fill=tk.X,pady=5,padx=5,side=tk.LEFT)
    ...
    versoin_la=ctk.CTkButton(fram_on_top_ver  ,  text="v0.5.1",state="disable",fg_color="#2BFED0",text_color="black")
    ...
    with open("client_set","r") as f:
        ...
        if client_set[15]=="false":
            sniffing_var=ctk.IntVar(value=0)
        elif  client_set[15]=="true":
            sniffing_var=ctk.IntVar(value=1)
    ...
    def save_client_setting(*args):
        settings = {
            "perapp": "false\n" if perapp_var.get() == 0 else "true\n",
            "enablelocaldns": "false\n" if enablelocaldns_var.get() == 0 else "true\n",
            "enablefakedns": "false\n" if enablefakedns_var.get() == 0 else "true\n",
            "local_dns_port": local.get().strip() + "\n" or "10853\n",
            "remote_dns": remotedns.get().strip() + "\n" or "https://8.8.8.8/dns-query\n",
            "domain_strategy": domainStrategy.get().strip() + "\n",
            "custom_proxy": customRules_proxy.get("1.0", tk.END).strip() + " \n",
            "custom_direct": customRules_direct.get("1.0", tk.END).strip() + " \n",
            "custom_blocked": customRules_blocked.get("1.0", tk.END).strip() + " \n",
            "socks5_port": socks5.get().strip() + "\n" or "10808\n",
            "http_port": http.get().strip() + "\n" or "10809\n",
            "allow_increase": "true\n" if allowincrease_var.get() == 1 else "false\n",
            "domestic_dns": domesticdns.get().strip() + "\n" or "1.1.1.2\n",
            "log_level": loglevel.get().strip() + "\n",
            "test_link": test_link.get().strip() + "\n" or "http://connectivitycheck.gstatic.com/generate_204\n",
            "sniffing": "false" if sniffing_var.get() == 0 else "true"
        }
        test_link_=settings["test_link"]
        enablefakedns_la.configure(state=tk.NORMAL if enablelocaldns_var.get() == 1 else tk.DISABLED)
        output_order = [
            "perapp",
            "enablelocaldns",
            "enablefakedns",
        ...
        output_lines = [settings[key] for key in output_order]
        with open("client_set", "w") as f:
    ...
    sniffing=ctk.CTkCheckBox(button_frame3 ,variable=sniffing_var,onvalue=1,offvalue=0, text="sniffing")
    ...
    def show_confs(test:str,is_new=False,len_f=0,is_dict=False):
            ...
            try:
                for i in conf_simple:
                    ...
                    if not is_new:
                        ...
                        frams_in_ping=[create_ping_label(frame_confs,i)]+frams_in_ping[:]
                        if LOC_user!=None:
        ...
        def create_ping_label(frame,i :str):
            if not isinstance(i , dict):
                ping_label = ctk.CTkLabel(frame, text=i.split(":")[-1].strip() if  ":" in i.split("#")[-1] else "", width=50,corner_radius=5,fg_color="#7E3612" if  ":" in i.split("#")[-1] else None)
            else:
                ping_label = ctk.CTkLabel(frame, text=i["remarks"].split(":")[-1].strip() if  ":" in i["remarks"].split("#")[-1] else "", width=50,corner_radius=5,fg_color="#7E3612" if  ":" in i["remarks"].split("#")[-1] else None)
    ...
    def th_ping_all(sus="", nume=0,is_new=False,is_all=False):
        ...
        def ping_all():
            ...
            def s_xray(conf_path,t):
                ...
                startupinfo = subprocess.STARTUPINFO()
                startupinfo.dwFlags |= subprocess.STARTF_USESHOWWINDOW
                proc=subprocess.Popen([xray_abs, 'run', '-c', conf_path], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, startupinfo=startupinfo)
                process_manager.add_process(f"xray_{t}", proc.pid)
            def stop_x(pidproc):
                subprocess.call(["taskkill", "/F", "/PID", str(pidproc)],creationflags=subprocess.CREATE_NO_WINDOW)
            def s_hy2(path_file,t):
                global hyproc
                startupinfo = subprocess.STARTUPINFO()
                startupinfo.dwFlags |= subprocess.STARTF_USESHOWWINDOW
                hy=subprocess.Popen (['hy2/hysteria.exe', 'client' ,'-c' , path_file], stdin=subprocess.DEVNULL, stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL,startupinfo=startupinfo)
                process_manager.add_process(f"hysteria_{t}", hy.pid)
            def stop_h(hyproc):
                subprocess.call(["taskkill", "/F", "/PID", str(hyproc)],creationflags=subprocess.CREATE_NO_WINDOW)
            def load_config():
                global count
            ...
            def process_ping(i:str, t,counter=2) :
                ...
                with open(path_test_file, "w") as f:
                    try:
                        if not is_dict:
                            f.write(parse_configs(i, cv=2 if sus!="" else t+2, hy2_path=hy2_path_test_file))
                        else:
                            json.dump(update_ip_addresses(i,2 if sus!="" else t+2), f)
                    except Exception:
                        is_wrong = True
                if not is_wrong:
                    ...
                    def pingg():
                        try:
                            proxies = {"http": f"http://127.0.0.{2 if sus!='' else t+2}:{port}",
                ...
                if is_dict:
                    with threading.Lock():
                        if t < len(copy_in_sus_nms):
                            copy_in_sus_nms[t]["remarks"] += f"{':'.join(copy_in_sus_nms[t]['remarks'].split(':')[:-1])}:{result}"
                else:
                    with threading.Lock():
                        copy_in_sus_nms[t] = f"{':'.join(copy_in_sus_nms[t].strip().split(':')[:-1]) if ':' in copy_in_sus_nms[t].strip().split('#')[-1] else copy_in_sus_nms[t].strip()}:{result}"
    ...
    def sort_frames_by_ping(frames: List, labels: List) -> List:
        ...
        numeric_pairs = [(frame, label) for frame, label in paired if extract_ping(label) is not None]
        non_numeric_pairs = [(frame, label) for frame, label in paired if extract_ping(label) is None]
        ...
        return [frame for frame, _ in sorted_positive_pairs] + [frame for frame, _ in sorted_negative_pairs] + [frame for frame, _ in non_numeric_pairs]
    def th_sort_all():
        def th():
    ...
    ping_b=ctk.CTkButton(dwonfram,command=lambda start=0:th_ping_all(nume=start,is_all=True),text="ping",corner_radius=20)
    ping_b.pack(side=tk.LEFT)
    sort_b=ctk.CTkButton(dwonfram,command=lambda start=0:th_sort_all(),text="sort",corner_radius=20)
