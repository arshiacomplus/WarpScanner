...
try:
    with open("D:/warp_setting", "r") as f:
        f.close()
except Exception:
    with open("D:/warp_setting", "a") as f:
        f.write("1")
        f.write("\n")
    ...
    # ctypes.windll.kernel32.SetFileAttributesW("D:/warp_setting" , 0x02)
...
def main(WH_ipVersion):
    ...
    def main_menu():
        ...
        def ping_ip(ip, port):
                ...
                if (float(icmp.avg_rtt) + icmp.jitter +  icmp.packet_loss )/3 > best_ip:
                    best_ip=(float(icmp.avg_rtt) +icmp.jitter + icmp.packet_loss )/3
                    best_result_avg=ip
        ...
        def check_ac_v6(mn):
            ...
            best_result=mn
            for ip, port, ping, loss_rate,jitter  in sorted_results[:10]:
                table.insert("","end", values=(ip, str(port) if port else "878", f"{ping:.2f}" if ping else "None", f"{loss_rate:.2f}%", f"{jitter:.2f}"))
            ...
            label_best.configure(state="disabled")
            best_result=2*[1]
            best_result[0]=f"{ip}"
            best_result[1]=878
    ...
    if WH_ipVersion !="ipv6":
        ...
        table=ttk.Treeview(rootmain, columns=("IP" ,"Port" , "Ping (ms)" , "Packet Loss (%)" ,"Jitter (ms)"), show='headings')
    ...
    table.heading("Jitter (ms)" , text="Jitter" )
    if WH_ipVersion != "ipv6":
    ...
    def wireguard_config_main(which_ip):
            ...
            def upload_to_bashupload(config_data):
                ...
                if response.ok:
                    ...
                    label_best.insert(1.0, str(download_link_with_query) )
                    label_best.configure(state="disabled")
                else:
            ...
            def check_ac_v6(mn, i):
                ...
                if i_ip_scan ==False:
                    ...
                    ip, port, ping, loss_rate,jitter = best_result
...
def mainactivity():
    ...
    if choose==1:
        ...
        try:
            with open("D:/nika.png", "r") as img_f:
                img_f.close()
        except FileNotFoundError:
            with open("D:/nika.png", "wb") as img_f:
                image_da= base64.b64decode(nika_img)
                img_f.write(image_da)
                ctypes.windll.kernel32.SetFileAttributesW("D:/nika.png" , 0x02)
        ...
        try:
            ...
            with open("D:/mahsa.png", "wb") as img_f:
    ...
    try:
            ...
            with open("D:/profile.png", "wb") as img_f:
                image_da= base64.b64decode(profile)
                img_f.write(image_da)
                ctypes.windll.kernel32.SetFileAttributesW("D:/profile.png" , 0x02)
                os.chmod("D:/profile.png" , 0o4444)
    menu_image =ImageTk.PhotoImage(Image.open(r"D:/profile.png"))
    ...
    def Setting():
        def sumbit_f():
            ...
            with open("D:/warp_setting" , "w") as f:
