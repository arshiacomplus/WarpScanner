...
def mainactivity():
    ...
    def parse_configs(conifg,num=0,cv=1):
        ...
        if not is_editing and ( isinstance(conifg, dict) or "{" in conifg ):
            ...
            def save_c(conf,num:int):
                global is_editing,frams_in_show
                is_editing=False
    ...
    def conc():
        ...
        def s_xray():
            ...
            with open("xray/xrayErr.log", "w") as errFile:
                subprocess.call([xray_abs, 'run', '-c' ,config_abs], stdin=subprocess.DEVNULL, stdout=errFile,stderr=errFile,creationflags=subprocess.CREATE_NO_WINDOW )
    ...
    def About():
        ...
        label_mew=ctk.CTkLabel(root_about,text="New version v0.4.5:\nGood news:\n\nbetter Graphic\n\nfix Bugs\n\nBad news:\n\nNotihng:))")
    ...
    def check_up():
            ...
            def on_go():
                ...
                if ret[0]  == True:
                    ...
                    label_up=ctk.CTkLabel(rootupdate ,font=("Helvetica", 12, "bold"), text="New version v0.4.5:\nGood news:\n\nbetter Graphic\n\nfix Bugs\n\nBad news:\n\nNotihng:))")
                    label_up.pack(fill=tk.X)
...
#     labelmain.pack()
    ...
    if list_freg[8]=="false":
        is_warp_on_on=ctk.IntVar(value=0)
    else:
        is_warp_on_on=ctk.IntVar(value=1)
    ...
    scrollable_vpn.pack(padx=10, pady=10, fill="both")
    ...
    is_warp_on_on.trace_add("write",save_frag_setting)
    ...
    domainStrategy=ctk.CTkOptionMenu(button_frame, values=domainStrategy_var,
                                                                 command=save_client_setting)
    ...
    customRules_direct.bind("<KeyRelease>", save_client_setting, add="+")
    ...
    customRules_blocked.bind("<KeyRelease>", save_client_setting, add="+")
    ####################   advanced settings   ########################
    ...
    def show_confs(test:str):
        ...
        def remove_frame(widget):
            try:
                ...
                pass
        def edit_config(conf,count:int):
            global is_editing
            is_editing=True
            is_dict=False
            with open("xray/"+selec_sub.get(),"r",encoding="utf-8") as f:
        ...
        def show_confs_therd():
            ...
            for i in conf_simple:
                ...
                frams_in_show.append(create_label(frame_confs, i_p))
                frams_in_ping.append(create_ping_label(frame_confs))
                create_buttons(frame_confs, i, count)
                ...
                if stoped_loop:
                    ...
                    frame_confs.bind("<Double-Button-1>",lambda e, i=i, is_on=is_on, count=count: th_ping_all(i, count))
                    break
        ...
        def create_label(frame, text):
            label = ctk.CTkTextbox(frame, width=100, height=48)
        ...
        def create_buttons(frame, config, count):
            ctk.CTkButton(frame, image=copy_icon, text="", width=25, command=lambda: copy_config(config), hover_color="#DF9C27", fg_color="#2b2b2b").pack(fill=tk.X, side=tk.LEFT, padx=(100, 0))
            ctk.CTkButton(frame, image=remove_icon, text="", width=25, command=lambda: remove_config(frame, count), hover_color="#DF9C27", fg_color="#2b2b2b").pack(fill=tk.X, side=tk.LEFT, padx=(10, 0))
            ctk.CTkButton(frame, image=edit_icon, text="", width=25, command=lambda: edit_config(config, count), hover_color="#DF9C27", fg_color="#2b2b2b").pack(fill=tk.X, side=tk.LEFT, padx=(10, 0))
    ...
    def th_ping_all(sus="", nume=0):
        ...
        def ping_all():
            ...
            def load_config():
                with open(f"xray/{selec_sub.get()}", "r", encoding="utf-8") as f:
                    if selec_sub.get() == "user_custom_confs":
                        ...
                        return remove_empty_strings(f.readlines()), False
            ...
            def process_ping(i, t,counter=2) :
