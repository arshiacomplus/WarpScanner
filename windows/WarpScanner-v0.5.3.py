...
DEFAULT_CONFIG = {
    ...
    "scanner_settings": {
        "scan_speed": "normal",
        "save_scan_result_method": "panel_vahid",
        "save_result_with_port": True,
        "warp_key_api": "api_2",
        "icmp": {
        "interval": 1,
        "count": 2,
        "timeout": 5,
        "acceptable_ping_range": 300
        },
        "output_format": {
        "sing_box": "hiddify_new",
        "v2ray": "v2rayn_pro_new"
        },
        "ipv4_scan_range_source": "warp",
        "ping_with_generated_config": "off",
        "custom_config_for_ping_test": ""
    },
    "ui_preferences": {
        "initial_tab": "main"
    },
...
Cpu_speed = app_config.get("scanner_settings", {}).get("scan_speed","normal")
best_ip = 0
do_you_save = ""
best_result_avg = ""
i_ip_scan = False
...
concted = False
ip_loc_img=None
ping_range_see = app_config.get("scanner_settings", {}).get("icmp",{}).get("acceptable_ping_range",300)
...
try:
    ...
    sCs=app_config["scanner_settings"]["icmp"]
    interval_see, count_see, timeout_see, ping_range_see = (
        float(str(sCs["interval"]).strip()),
        int(str(sCs["count"]).strip()),
        float(str(sCs["timeout"]).strip()),
        int(str(sCs["acceptable_ping_range"]).strip())
    )
except Exception as E:
    messagebox.showerror("Error", f"An error has occurred!: \n {E}")
...
def free_cloudflare_account():
    if app_config["scanner_settings"]["warp_key_api"] == "api_2":
        keys = bind_keys()
        return keys
...
def generate_wireguard_url(configure, endpoint):
    ...
    if not all(
        ...
        return None
    if not app_config["scanner_settings"]["warp_key_api"] == "api_2":
        encoded_addresses = [quote(address1) for address1 in (configure["Address"])]
        address = ",".join(encoded_addresses)
        if not app_config["scanner_settings"]["output_format"]["v2ray"]=="v2rayn_pro_new":
            wireguard_urll = (
...
def mainactivity():
        ...
        if is_editing:
            ...
            def save_normal_conf_form(conifg: str, num: int, remarks: ctk.CTkEntry):
                ...
                data = {
                    ...
                    "scy": security.get().strip() if security else "",
            ...
            remarks.pack(fill=tk.X, side=tk.LEFT, padx=4,expand=True,anchor="w")
            ...
            ip_address.pack(fill=tk.X, side=tk.LEFT, padx=2,expand=True,anchor="w")
            ...
            ports.pack(fill=tk.X, side=tk.LEFT, padx=4,expand=True,anchor="w")
            ...
            def set_type():
                ...
                network.pack(fill=tk.X, side=tk.LEFT, padx=4,expand=True,anchor="w")
            ...
            def check_type(typ: str):
                ...
                for label_widget, entry in zip(
                    ...
                    entry.pack(fill=tk.X, side=tk.LEFT, padx=4,expand=True,anchor="w")
            ...
            def set__security():
                ...
                tls.pack(fill=tk.X, side=tk.LEFT, padx=4,expand=True,anchor="w")
            ...
            def check_security(sec: str):
                ...
                for label, other in zip(shows_widet["label"], shows_widet["other"]):
                    ...
                    other.pack(fill=tk.X, side=tk.LEFT, padx=4,expand=True,anchor="w")
            ...
            r_host_la = ctk.CTkLabel(fram_in_in_5, text="requests host: ")
            r_host = ctk.CTkEntry(
            ...
            if conifg.startswith("vless://") or conifg.startswith("vmess://"):
                ...
                id.pack(fill=tk.X, side=tk.LEFT, padx=2,expand=True,anchor="w")
                if conifg.startswith("vless://"):
                    ...
                    flow.pack(fill=tk.X, side=tk.LEFT, padx=4,expand=True,anchor="w")
                    encryption_la = ctk.CTkLabel(fram_in_in_3, text="encryption: ")
                    encryption_la.pack(padx=(20, 0), side=tk.LEFT)
                    encryption.pack(fill=tk.X, side=tk.LEFT, padx=4,expand=True,anchor="w")
                    set_type()
                    check_type(TYPE)
                    set__security()
                    check_security(SECURITY)
                if conifg.startswith("vmess://"):
                    ...
                    alterid.pack(fill=tk.X, side=tk.LEFT, padx=4,expand=True,anchor="w")
                    ...
                    security.pack(fill=tk.X, side=tk.LEFT, padx=4,expand=True,anchor="w")
                    set_type()
                    check_type(TYPE)
                    set__security()
                    check_security(SECURITY)
                ...
                pass_w.pack(fill=tk.X, side=tk.LEFT, padx=2,expand=True,anchor="w")
                set_type()
                check_type(TYPE)
                set__security()
                check_security(SECURITY)
                ...
                security_la = ctk.CTkLabel(fram_in_in_3, text="security: ")
                security_la.pack(padx=(20, 0), side=tk.LEFT)
                security.configure(values=SECURITY_w)
                security.pack(fill=tk.X, side=tk.LEFT, padx=4,expand=True,anchor="w")
                ...
                user.pack(fill=tk.X, side=tk.LEFT, padx=2,expand=True,anchor="w")
            elif conifg.startswith("wireguard://"):
                ...
                secrekey.pack(fill=tk.X, side=tk.LEFT, padx=2,expand=True,anchor="w")
                fram_in_in_3.pack(fill=tk.X, pady=5, ipady=5, padx=5)
                ...
                pbk.pack(fill=tk.X, side=tk.LEFT, padx=2,expand=True,anchor="w")
                ...
                reserved.pack(fill=tk.X, side=tk.LEFT, padx=2,expand=True,anchor="w")
                ...
                address.pack(fill=tk.X, side=tk.LEFT, padx=2,expand=True,anchor="w")
                ...
                mtu.pack(fill=tk.X, side=tk.LEFT, padx=2,expand=True,anchor="w")
                fram_in_in_6.pack(fill=tk.X, pady=5, ipady=5, padx=5)
                ...
                keepAlive.pack(fill=tk.X, side=tk.LEFT, padx=2,expand=True,anchor="w")
                ...
                wnoise.pack(fill=tk.X, side=tk.LEFT, padx=2,expand=True,anchor="w")
                fram_in_in_7 = ctk.CTkFrame(
                    fram_in,
                    height=100,
                    border_width=1,
                ...
                wnoisecount.pack(fill=tk.X, side=tk.LEFT, padx=2,expand=True,anchor="w")
                ...
                wnoisedelay.pack(fill=tk.X, side=tk.LEFT, padx=2,expand=True,anchor="w")
                fram_in_in_8 = ctk.CTkFrame(
                    fram_in,
                    height=100,
                    border_width=1,
                ...
                wpayloadsize.pack(fill=tk.X, side=tk.LEFT, padx=2,expand=True,anchor="w")
            elif conifg.startswith("hy2://") or conifg.startswith("hysteria2://"):
                pass_w_la = ctk.CTkLabel(fram_in_in_2, text="password: ")
                pass_w_la.pack(padx=2, side=tk.LEFT)
                pass_w.pack(fill=tk.X, side=tk.LEFT, padx=2,expand=True,anchor="w")
                ...
                obfs_password.pack(fill=tk.X, side=tk.LEFT, padx=4,expand=True,anchor="w")
                porthopinginterval_la.pack(padx=(20, 0), side=tk.LEFT)
                porthopinginterval.pack(fill=tk.X, side=tk.LEFT, padx=4,expand=True,anchor="w")
                tls_la = ctk.CTkLabel(fram_in_in_6, text="tls: ")
                tls_la.pack(padx=(20, 0), side=tk.LEFT)
                tls.pack(fill=tk.X, side=tk.LEFT, padx=4,expand=True,anchor="w")
                fram_in_in_6.pack(fill=tk.X, pady=5, ipady=5, padx=5)
                ...
                if SECURITY == "tls":
                    ...
                    sni.pack(fill=tk.X, side=tk.LEFT, padx=4,expand=True,anchor="w")
                    fram_in_in_7.pack(fill=tk.X, pady=5, ipady=5, padx=5)
                    insecure_la.pack(padx=(20, 0), side=tk.LEFT)
                    insecure.pack(fill=tk.X, side=tk.LEFT, padx=4,expand=True,anchor="w")
                    pinsha256_la.pack(padx=(20, 0), side=tk.LEFT)
                    pinsha256.pack(fill=tk.X, side=tk.LEFT, padx=4,expand=True,anchor="w")
            is_editing = False
            return
        ...
        if ttemp:
            if ttemp[0] != "Unavailable":
                ...
                try:
                    ...
                    ip_loc = False
        if ip_loc != False:
            ipp_la.configure(image=ip_loc_img, text="")
        return result
    ...
    def conc():
        ...
        if concted:
            ...
            ping_la.configure(text="click to connect ")
            return
    ...
    def goCheckWithConfig(sorted_results):
        ...
        def go():
            ...
            config = app_config["scanner_settings"]["custom_config_for_ping_test"].strip()
            http5 = app_config["core"]["inbound_ports"]["http"]
    ...
    def wireguard_config_main(which_ip):
        ...
        def check_ac_v6(mn, i):
            ...
            if is_sub == False and is_v2ray == False:
                outfm=app_config["scanner_settings"]["output_format"]["sing_box"]
                conf = ""
                if not outfm=="hiddify_new":
                    conf = temp_sing_old
                else:
                    conf = temp_sing_new
        ...
        if i_ip_scan:
            ...
            if which_ip == "ipv6":
                ...
                for result in results:
                    ...
                    if loss_rate == 0.00 and ping != 0.0 and ping < ping_range_see:
                        try:
                            ...
                            if app_config["scanner_settings"]["save_scan_result_method"] == "panel_bpb":
                                save_result.append(",")
                ...
                if app_config["scanner_settings"]["ping_with_generated_config"] != "off":
        ...
        def main_menu():
            ...
            if WH_ipVersion == "ipv6":
                ...
                if app_config["scanner_settings"]["ping_with_generated_config"] != "off":
                    extended_results = goCheckWithConfig(extended_results)
                sorted_results = sorted(extended_results, key=lambda x: x[5])
                if app_config["scanner_settings"]["save_scan_result_method"] != "with_score":
                    for (
                        ip,
                        port,
                        ping,
                        ...
                        if loss_rate == 0.00 and ping != 0.0 and ping < ping_range_see:
                            if not app_config["scanner_settings"]["save_result_with_port"]:
                                try:
                                    save_result.index(str(ip))
                                except Exception:
                                    if app_config["scanner_settings"]["save_scan_result_method"] == "panel_bpb":
                                ...
                                try:
                                    ...
                                    if app_config["scanner_settings"]["save_scan_result_method"] == "panel_bpb":
                                        ...
                                        save_result.append("\n")
                                        save_result.append(str(ip) + ":" + str(port))
                elif app_config["scanner_settings"]["save_scan_result_method"] == "with_score":
                    for (
                        ip,
                        port,
                        ping,
                ...
                start_ips = []
                end_ips = []
                ip_type=app_config.get("scanner_settings", {}).get("ipv4_scan_range_source","warp")
                if ip_type == "warp":
    ...
    def main_move():
        ...
        if radio_var.get() == 2:
            ...
            if option == "wireguard configure for hiddify":
                theard_wireguard_config_ok()
            elif option == "configure without ip scan for hiddify":
                theard_wireguard_config_without()
            elif option == "wireguard with a sub link for hiddify":
    ...
    def clean():
        try:
            ...
            separator = (
                "," if  app_config["scanner_setting"]["save_scan_result_method"] == "panel_bpb" else "\n"
            )
            clean_result = []
            ip_count = 0
            for line in lines:
    ...
    tabview.add("configs")
    tabview.add("table")
    if app_config["ui_preferences"]["initial_tab"] == "main":
    ...
    def Setting():
        ...
        def submit_f():
            global app_config
            global interval_see, count_see, timeout_see, ping_range_see

            if "scanner_settings" not in app_config: app_config["scanner_settings"] = {}
            scanner_cfg = app_config["scanner_settings"]

            scanner_cfg["scan_speed"] = "faster" if int(checkbox_var.get()) == 1 else "normal"
            is_none_or_not=checkbox_var2.get()
            save_method_val = checkbox_var3.get()
            if save_method_val == 1:
                scanner_cfg["save_scan_result_method"] = "panel_bpb"
            elif save_method_val == 0:
                scanner_cfg["save_scan_result_method"] = "panel_vahid"
            else:
                scanner_cfg["save_scan_result_method"] = "with_score"
            if is_none_or_not==0:
                scanner_cfg["save_scan_result_method"] = "none"
            scanner_cfg["warp_key_api"] = "api_1" if checkbox_var4.get() == 1 else "api_2"
            scanner_cfg["save_result_with_port"] = True if checkbox_var10.get() == 1 else False

            if "icmp" not in scanner_cfg: scanner_cfg["icmp"] = {}
            try:
                scanner_cfg["icmp"]["interval"] = float(interval.get().strip())
            except ValueError:
                scanner_cfg["icmp"]["interval"] = DEFAULT_CONFIG["scanner_settings"]["icmp"]["interval"]
            try:
                scanner_cfg["icmp"]["count"] = int(count_icmp_entry.get().strip())
            except ValueError:
                scanner_cfg["icmp"]["count"] = DEFAULT_CONFIG["scanner_settings"]["icmp"]["count"]
            try:
                scanner_cfg["icmp"]["timeout"] = float(timeout.get().strip())
            except ValueError:
                scanner_cfg["icmp"]["timeout"] = DEFAULT_CONFIG["scanner_settings"]["icmp"]["timeout"]
            try:
                scanner_cfg["icmp"]["acceptable_ping_range"] = int(ping_range.get().strip())
            except ValueError:
                scanner_cfg["icmp"]["acceptable_ping_range"] = DEFAULT_CONFIG["scanner_settings"]["icmp"]["acceptable_ping_range"]

            if "output_format" not in scanner_cfg: scanner_cfg["output_format"] = {}
            scanner_cfg["output_format"]["sing_box"] = "hiddify_old_singbox" if checkbox_var8.get() == 1 else "hiddify_new"
            scanner_cfg["output_format"]["v2ray"] = "v2rayn_pro_old" if checkbox_var9.get() == 1 else "v2rayn_pro_new"

            scanner_cfg["ipv4_scan_range_source"] = "warp" if checkbox_var12.get() == 1 else "gcore"
            scanner_cfg["ping_with_generated_config"] = str(switch.get()).lower()
            scanner_cfg["custom_config_for_ping_test"] = configtextbox.get("1.0", tk.END).strip()
            if "theme" not in app_config: app_config["theme"] = {}
            theme_val = checkbox_var11.get()
            if theme_val == 1:
                app_config["theme"]["type"] = "system"
            elif theme_val == 0:
                app_config["theme"]["type"] = "dark"
            else:
                app_config["theme"]["type"] = "light"
            # ctk.set_appearance_mode(app_config["theme"]["type"])
            if "ui_preferences" not in app_config: app_config["ui_preferences"] = {}
            app_config["ui_preferences"]["initial_tab"] = "main" if checkbox_var15.get() == 1 else "vpn"
            save_app_config(app_config)
            interval_see = app_config["scanner_settings"]["icmp"]["interval"]
            count_see = app_config["scanner_settings"]["icmp"]["count"]
            timeout_see = app_config["scanner_settings"]["icmp"]["timeout"]
            ping_range_see = app_config["scanner_settings"]["icmp"]["acceptable_ping_range"]
            labe.place_forget()
            toggle_menu_button.configure(text="Menu")
        ...
        scrollable_frame.place(x=tabview.tab("main").winfo_width() - (170 + 20), y=0)
        ...
        labe.place(y=0, x= (tabview.tab("main").winfo_width() - (170 + 20)) - 30 if scrollable_frame.winfo_ismapped() else rootmainactivity.winfo_width() - 50)
        labe.bind("<Button-1>", event_submit_f)
        labe.lift()


        scanner_prefs = app_config.get("scanner_settings", DEFAULT_CONFIG.get("scanner_settings", {}))
        theme_prefs = app_config.get("theme", DEFAULT_CONFIG.get("theme", {}))
        ui_prefs = app_config.get("ui_preferences", DEFAULT_CONFIG.get("ui_preferences", {}))



        spu_label = ctk.CTkLabel(scrollable_frame, compound="right", text="Scan speed ")
        spu_label.pack(fill=tk.X)
        scan_speed_json = scanner_prefs.get("scan_speed", "normal")
        checkbox_var = ctk.IntVar(value=1 if scan_speed_json == "faster" else 0)
        ...
        cd_gcore_label = ctk.CTkLabel(scrollable_frame, compound="right", text="Which IP range (IPv4)")
        cd_gcore_label.pack(fill=tk.X)
        current_ipv4_source = scanner_prefs.get("ipv4_scan_range_source", "warp")
        checkbox_var12 = ctk.IntVar(value=1 if current_ipv4_source == "warp" else 0)
        ...
        save_ch_label = ctk.CTkLabel(scrollable_frame, compound="right", text="Save result")
        save_ch_label.pack(fill=tk.X)
        save_method_json = scanner_prefs.get("save_scan_result_method", "panel_vahid")

        checkbox_var2 = ctk.IntVar(value=1 if save_method_json and save_method_json.lower() != "none" and save_method_json != "" else 0)
        checkbox3 = ctk.CTkCheckBox(scrollable_frame, text="Yes (Enable Save)", variable=checkbox_var2, onvalue=1, offvalue=0)
        checkbox3.bind("<Enter>", hand2)
        checkbox4 = ctk.CTkCheckBox(scrollable_frame, text="No (Disable Save)", variable=checkbox_var2, onvalue=0, offvalue=0)
        checkbox4.bind("<Enter>", hand2)
        pack_widgets([checkbox3, checkbox4], ipady=5, fill=tk.X)

        which_panel_label = ctk.CTkLabel(scrollable_frame, compound="right", text="Save Format/Panel")
        which_panel_label.pack(fill=tk.X)
        if save_method_json == "panel_bpb": checkbox_var3_val = 1
        elif save_method_json == "panel_vahid": checkbox_var3_val = 0
        else: checkbox_var3_val = 2
        checkbox_var3 = ctk.IntVar(value=checkbox_var3_val)
        ...
        which_tab_label = ctk.CTkLabel(scrollable_frame, compound="right", text="First Tab")
        which_tab_label.pack(fill=tk.X)
        current_initial_tab = ui_prefs.get("initial_tab", "main")
        checkbox_var15 = ctk.IntVar(value=1 if current_initial_tab == "main" else 0)
        checkbox23 = ctk.CTkCheckBox(scrollable_frame, text="Main Tab", variable=checkbox_var15, onvalue=1, offvalue=0)
        checkbox23.bind("<Enter>", hand2)
        checkbox24 = ctk.CTkCheckBox(scrollable_frame, text="VPN Tab", variable=checkbox_var15, onvalue=0, offvalue=0)
        checkbox24.bind("<Enter>", hand2)
        pack_widgets([checkbox23, checkbox24], ipady=5, fill=tk.X)



        which_theme_label = ctk.CTkLabel(scrollable_frame, compound="right", text="Theme")
        which_theme_label.pack(fill=tk.X)
        current_theme = theme_prefs.get("type", "system")
        if current_theme == "system": checkbox_var11_val = 1
        elif current_theme == "dark": checkbox_var11_val = 0
        else: checkbox_var11_val = 2
        checkbox_var11 = ctk.IntVar(value=checkbox_var11_val)
        ...
        is_port_la_label = ctk.CTkLabel(scrollable_frame, compound="right", text="Save Result with Port")
        is_port_la_label.pack(fill=tk.X)
        save_with_port_enabled = scanner_prefs.get("save_result_with_port", True)
        checkbox_var10 = ctk.IntVar(value=1 if save_with_port_enabled else 0)
        ...
        Wh_api_label = ctk.CTkLabel(scrollable_frame, compound="right", text="WARP Key API")
        Wh_api_label.pack(fill=tk.X)
        current_warp_api = scanner_prefs.get("warp_key_api", "api_2")



        checkbox_var4 = ctk.IntVar(value=1 if current_warp_api == "api_1" else 0)
        checkbox8 = ctk.CTkCheckBox(scrollable_frame, command=api_no, text="API 1 (Not Recommended)", variable=checkbox_var4, onvalue=1, offvalue=0)
        checkbox8.bind("<Enter>", hand2)
        checkbox9 = ctk.CTkCheckBox(scrollable_frame, text="API 2 (Recommended)", variable=checkbox_var4, onvalue=0, offvalue=1)
        checkbox9.bind("<Enter>", hand2)
        pack_widgets([checkbox8, checkbox9], ipady=5, fill=tk.X)



        save_ch_label_singbox = ctk.CTkLabel(scrollable_frame, compound="right", text="Sing-box Output Format")
        save_ch_label_singbox.pack(fill=tk.X)
        current_singbox_format = scanner_prefs.get("output_format", {}).get("sing_box", "hiddify_new")
        checkbox_var8 = ctk.IntVar(value=1 if current_singbox_format == "hiddify_old_singbox" else 0)
        checkbox10 = ctk.CTkCheckBox(scrollable_frame, text="Hiddify (Old) + Sing-box", variable=checkbox_var8, onvalue=1, offvalue=0)
        checkbox10.bind("<Enter>", hand2)
        checkbox11 = ctk.CTkCheckBox(scrollable_frame, text="Hiddify (Newer)", variable=checkbox_var8, onvalue=0, offvalue=0)
        checkbox11.bind("<Enter>", hand2)
        pack_widgets([checkbox10, checkbox11], ipady=5, fill=tk.X)



        save_ch2_label_v2ray = ctk.CTkLabel(scrollable_frame, compound="right", text="V2Ray Output Format")
        save_ch2_label_v2ray.pack(fill=tk.X)
        current_v2ray_format = scanner_prefs.get("output_format", {}).get("v2ray", "v2rayn_pro_new")
        checkbox_var9 = ctk.IntVar(value=1 if current_v2ray_format == "v2rayn_pro_old" else 0)
        ...
        icmp_prefs_json = scanner_prefs.get("icmp", DEFAULT_CONFIG["scanner_settings"].get("icmp", {}))

        ping_lab_label = ctk.CTkLabel(scrollable_frame, compound="right", text="Ping Range (ms)")
        ping_lab_label.pack(fill=tk.X)

        global ping_range
        ping_range = ctk.CTkEntry(scrollable_frame, placeholder_text="e.g., 300", width=25)
        ping_range.insert(0, str(icmp_prefs_json.get("acceptable_ping_range", 300)))
        ping_range.pack(padx=10, pady=(0,10), fill=tk.X)

        advane_label = ctk.CTkLabel(scrollable_frame, text="<Advanced ICMP Settings>\n")
        advane_label.pack(fill=tk.X)

        icmp_interval_label = ctk.CTkLabel(scrollable_frame, compound="right", text="ICMP Interval (s)")
        icmp_interval_label.pack(fill=tk.X)

        global interval
        interval = ctk.CTkEntry(scrollable_frame, placeholder_text="e.g., 0.2 or 1", width=25)
        interval.insert(0, str(icmp_prefs_json.get("interval", 1.0)))
        interval.pack(padx=10, pady=(0,10), fill=tk.X)

        icmp_count_label = ctk.CTkLabel(scrollable_frame, compound="right", text="ICMP Ping Count")
        icmp_count_label.pack(fill=tk.X)

        global count_icmp_entry
        count_icmp_entry = ctk.CTkEntry(scrollable_frame, placeholder_text="e.g., 2 or 4", width=25)
        count_icmp_entry.insert(0, str(icmp_prefs_json.get("count", 2)))
        count_icmp_entry.pack(padx=10, pady=(0,10), fill=tk.X)

        icmp_timeout_label = ctk.CTkLabel(scrollable_frame, compound="right", text="ICMP Timeout (s)")
        icmp_timeout_label.pack(fill=tk.X)

        global timeout
        timeout = ctk.CTkEntry(scrollable_frame, placeholder_text="e.g., 1 or 5", width=25)
        timeout.insert(0, str(icmp_prefs_json.get("timeout", 5.0)))
        timeout.pack(padx=10, pady=(0,10), fill=tk.X)



        ping_with_config_json = scanner_prefs.get("ping_with_generated_config", "off")

        global switch_var
        switch_var = ctk.StringVar(value=ping_with_config_json)

        global switch
        switch = ctk.CTkSwitch(scrollable_frame, text="Ping with config", variable=switch_var, onvalue="on", offvalue="off")
        switch.pack(pady=5, ipady=5, fill=tk.X)



        custom_ping_test_config_json = scanner_prefs.get("custom_config_for_ping_test", "")

        global configtextbox
        configtextbox = ctk.CTkTextbox(scrollable_frame, height=100)
        configtextbox.insert("1.0", custom_ping_test_config_json)
        configtextbox.pack(pady=(0,5),fill=tk.X)




        submit_button = ctk.CTkButton(scrollable_frame, text="Save Settings", command=submit_f)
        submit_button.pack(pady=5, ipady=5, fill=tk.X)
        submit_button.bind("<Enter>", hand2)
    ...
    def need_how_meny(*args):
        if optionmenu.get() == "wireguard with a sub link for hiddify":
            forget_widget_list([button, button1, button3])
            how_menypp.pack(fill=tk.X, anchor="center")
            button.bind("<Enter>", hand2)
            button1.bind("<Enter>", hand2)
    ...
    label_best = ctk.CTkTextbox(tabview.tab("main"), width=50, height=1,fg_color=("white", "#1f1f1f"),border_color=("gray70", "gray30"),border_width=1
)
    ...
    optionmenu = ctk.CTkOptionMenu(
        tabview.tab("main"),
        values=[
            "wireguard configure for hiddify",
            "configure without ip scan for hiddify",
            "wireguard with a sub link for hiddify",
    ...
    def is_frag_or_wire(nn):
        ...
        if op == "wiregaurd config setting":
            ...
            enablelocaldns_la.pack(
                    padx=(5,0),pady=(5, 5),
                    anchor="w",
                    side=tk.LEFT,
                    expand=True
                )
            enablefakedns_la.pack(
                    padx=(5,0),pady=(5, 5),
                    anchor="w",
                    side=tk.LEFT,
                    expand=True
            ...
            customRules_proxy_la.pack(
                    padx=(5,0),pady=(5, 5),
                    anchor="w",
                    side=tk.LEFT
                )
            customRules_proxy.pack(
                    padx=(20, 5),
                    anchor="w",
                    fill=tk.X,
                    side=tk.LEFT,
            ...
            customRules_direct_la.pack(
                    padx=(5,0),pady=(5, 5),
                    anchor="w",
                    side=tk.LEFT
                )
            customRules_direct.pack(
                    padx=(20, 5),
                    anchor="w",
                    fill=tk.X,
                    side=tk.LEFT,
            ...
            customRules_blocked_la.pack(
                    padx=(5,0),pady=(5, 5),
                    anchor="w",
                    side=tk.LEFT
                )
            customRules_blocked.pack(
                    padx=(20, 5),
                    anchor="w",
                    fill=tk.X,
                    side=tk.LEFT,
    ...
    if not app_config.get("core", {}).get("fragment",{}).get("enabled",False):
    ...
    if not app_config.get("warp_on_warp",{}).get("enabled",False):
        print(app_config.get("warp_on_warp",{}).get("enabled",False))
    ...
    pill_frame = ctk.CTkFrame(
        ...
        corner_radius=20,
        border_width=1,
        fg_color=("gray95", "#2F2F2F"),
        border_color=("gray70", "gray30")
    )
    pill_frame.pack(pady=15, padx=20, fill="x")

    pill_frame.grid_columnconfigure(0, weight=1)
    pill_frame.grid_columnconfigure(1, weight=0)


    ping_la = ctk.CTkLabel(
        pill_frame,
    ...
    ping_la.grid(row=0, column=0, sticky="w", padx=(15, 5), pady=10)
    location_frame = ctk.CTkFrame(
        pill_frame,
        fg_color="transparent"
    )
    location_frame.grid(row=0, column=1, sticky="e", padx=(5, 15))

    location_icon = ctk.CTkLabel(
        location_frame,
        text="",
    )
    location_icon.grid(row=0, column=0, padx=(0, 3))
    ipp_la = ctk.CTkLabel(
        location_frame,
        text="None",
    ...
    is_warp_or_not.pack(fill=tk.X, pady=3,padx=5)
    set = [
        "tlshello\n",
        "10-30\n",
        "1-5\n",
    ...
    button_frame.pack(pady=8, padx=3, fill="x")
    ...
    button_frame2.pack(pady=8, padx=3, fill="x")
    ...
    button_frame3.pack(pady=8, padx=3, fill="x", ipady=5)
    ...
    mux = ctk.CTkEntry(
        button_frame3,
        textvariable=mux_var_str,
        placeholder_text="mux[5]",
        height=28
    )
    ...
    def show_confs(test: str, is_new=False, len_f=0, is_dict=False):
        ...
        def create_ping_label(frame, i: str):
            ...
            txt_color="white"
            if ":" in remarks:
                ping_text = str(remarks.split(":")[-1].strip())
                if ping_text.isdigit():
    ...
    def th_ping_all(sus="", nume=0, is_new=False, is_all=False):
        ...
        def ping_all():
            ...
            def update_vmess_config_name_with_ping(vmess_link: str, ping_result: str) -> str:
                ...
                try:
                    # جدا کردن بخش اصلی لینک از تگ خارجی (بعد از #)
                    ...
                    # اضافه کردن padding اگر لازم باشد (برای دیکود صحیح Base64)
                    ...
                    # گرفتن نام فعلی کانفیگ (ps)
                    current_ps = config_dict.get("ps", "")

                    # حذف نتیجه پینگ قبلی اگر وجود دارد
                    base_ps = ':'.join(current_ps.split(':')[:-1]) if ':' in current_ps and current_ps.split(':')[-1].isdigit() or current_ps.split(':')[-1] == "-1" else current_ps

                    # اضافه کردن نتیجه پینگ جدید
                    updated_ps = f"{base_ps}:{ping_result}"
                    config_dict["ps"] = updated_ps.strip(':') # اگر base_ps خالی بود، : اضافه نشود

                    # تبدیل دیکشنری به‌روز شده به رشته JSON
                    updated_json_str = json.dumps(config_dict, separators=(',', ':')) # separators برای حذف فاصله‌های اضافی

                    # انکود کردن Base64 رشته JSON جدید
