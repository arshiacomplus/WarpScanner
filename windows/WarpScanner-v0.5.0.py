    ...
    def check_up():
            ...
            def on_go():
                ...
                if ret[0]  == True:
                    ...
                    label_up=ctk.CTkLabel(rootupdate ,font=("Helvetica", 12, "bold"), text="New version v0.5.0:\nGood news:\n\nbetter Graphic\n\nfix Bugs\n\nBad news:\n\nNotihng:))")
                    label_up.pack(fill=tk.X)
...
#     labelmain.pack()
    ...
    versoin_la=ctk.CTkButton(fram_on_top_ver  ,  text="v0.5.0",state="disable",fg_color="#2BFED0",text_color="black")
    ...
    def th_ping_all(sus="", nume=0,is_new=False,is_all=False):
        ...
        def ping_all():
            ...
            def s_xray(conf_path,t):
                global pidproc
                proc=subprocess.Popen([xray_abs, 'run', '-c', conf_path], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, creationflags=subprocess.CREATE_NO_WINDOW)
                process_manager.add_process(f"xray_{t}", proc.pid)
            def stop_x(pidproc):
                os.kill(pidproc,signal.SIGTERM)
            def s_hy2(path_file,t):
                global hyproc
                hy=subprocess.Popen (['hy2/hysteria.exe', 'client' ,'-c' , path_file], stdin=subprocess.DEVNULL, stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL,creationflags=subprocess.CREATE_NO_WINDOW)
